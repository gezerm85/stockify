...
export default function ExcelUploader() {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        console.log("Excel verisinin ilk satırı:", jsonData[0]); // Başlıkları kontrol edin
        setData(jsonData);
        setModalOpen(true);
      };
  ...
  // Firebase Firestore'a veri gönderimi: Gerçek gönderim işlemi
  const handleSend = async () => {
    // Excel verisini işleyelim: Farklı varyasyonları göz önünde bulundurarak
    ...
    try {
      ...
      const colRef = collection(db, "urunler"); // "urunler" koleksiyonu
      processedData.forEach((item) => {
        const docRef = doc(colRef, item.productCode); // Otomatik ID oluşturur
        batch.set(docRef, item);
      });
      await batch.commit();
      console.log("Veriler Firebase'e başarıyla gönderildi:", processedData);
      setLog(JSON.stringify(processedData, null, 2));
      setModalOpen(false);
    } catch (error) {
      console.error("Firebase'e veri gönderilirken hata:", error);
      setLog("Firebase'e veri gönderilirken hata: " + error.toString());
    }
  };
